  ...
  for y in range(1, len(input)-1):
    for x in range(1, len(input[y])-1):
      if input[y][x] > 9: # go through each entry and see if there are any above 9
        c9 += 1
  phase2() if c9 != 0 else phase3() # repeat phase 2 if there are values above 9, in order to create each possible flash
  ...
  for y in range(1, len(input)-1):
    for x in range(1, len(input[0])-1):
      if flashed[(x, y)] == True:
        flashed[(x, y)] = False
        input[y][x] = 0
        fcount[0] += 1 # modified from part 2 to see if 

while fcount[0] != ((len(input)-2)*(len(input[1])-2)): # iterate while the amount of flashes is less than the total number of octopuses
  phase1()
  i += 1
